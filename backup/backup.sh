...
MYIP=$(curl -sS ipv4.icanhazip.com)
NameUser=$(curl -sS https://raw.githubusercontent.com/keluargacobain/permission/main/ipmini | grep $MYIP | awk '{print $2}')
...
cp -r /var/lib/cobainvpn-pro/ /root/backup/cobainvpn-pro &> /dev/null
...
LLatest=`date`
Get_Data () {
git clone https://github.com/keluargacobain/backupusercobain.git /root/user-backup/ &> /dev/null
}
...
Save_And_Exit () {
    ...
    git remote add origin https://github.com/keluargacobain/backupusercobain
    git push -f https://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@github.com/keluargacobain/backupusercobain.git &> /dev/null
}
...
link="https://raw.githubusercontent.com/keluargacobain/backupusercobain/main/$NameUser/$NameUser.zip"
sleep 1
