...
colornow=$(cat /etc/cobainvpn/theme/color.conf)
NC="\e[0m"
COLOR1="$(cat /etc/cobainvpn/theme/$colornow | grep -w "TEXT" | cut -d: -f2|sed 's/ //g')"
COLBG1="$(cat /etc/cobainvpn/theme/$colornow | grep -w "BG" | cut -d: -f2|sed 's/ //g')"

APIGIT=$(cat /etc/cobainvpn/github/api)
EMAILGIT=$(cat /etc/cobainvpn/github/email)
USERGIT=$(cat /etc/cobainvpn/github/username)
...
if [[ -f /etc/cobainvpn/github/api && -f /etc/cobainvpn/github/email && /etc/cobainvpn/github/username ]]; then
   rec="OK"
else
    mkdir /etc/cobainvpn/github > /dev/null 2>&1
fi
...
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}" 
echo -e "$COLOR1┌────────────────────── BY ───────────────────────┐${NC}"
echo -e "$COLOR1│${NC}                • COBAIN TEAM •                  $COLOR1│$NC"
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}" 
echo -e ""
...
sleep 2
echo "$EMAIL1" > /etc/cobainvpn/github/email
echo "$USERNAME1" > /etc/cobainvpn/github/username
echo "$API1" > /etc/cobainvpn/github/api
echo "ON" > /etc/cobainvpn/github/gitstat
clear
echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}"
...
function resetipvps(){
clear
rm -f /etc/cobainvpn/github/email
rm -f /etc/cobainvpn/github/username
rm -f /etc/cobainvpn/github/api
rm -f /etc/cobainvpn/github/gitstat
echo "OFF" > /etc/cobainvpn/github/gitstat
echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}"
...
Isadmin=$(curl -sS https://raw.githubusercontent.com/keluargacobain/permission/main/ipmini | grep $MYIP | awk '{print $5}')
if [ "$Isadmin" = "OFF" ]; then
clear
echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}"
...
echo -e "$COLOR1│${NC} [INFO] Langsung Kesini t.me/ajikcobain/"
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}" 
echo -e "$COLOR1┌────────────────────── BY ───────────────────────┐${NC}"
echo -e "$COLOR1│${NC}                • COBAIN TEAM •                  $COLOR1│$NC"
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}" 
...
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}"
echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}"
GITREQ=/etc/cobainvpn/github/gitstat
if [ -f "$GITREQ" ]; then
    cekk="ok"
else 
    mkdir /etc/cobainvpn/github
    touch /etc/cobainvpn/github/gitstat
    echo "OFF" > /etc/cobainvpn/github/gitstat
fi

stst1=$(cat /etc/cobainvpn/github/gitstat)
if [ "$stst1" = "OFF" ]; then
clear
echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}"
echo -e "$COLOR1│${NC} ${COLBG1}               • REGISTER IPVPS •              ${NC} $COLOR1│$NC"
...
stst=$(cat /etc/cobainvpn/github/gitstat)
if [ "$stst" = "ON" ]; then
APIOK="CEK API"
rex="viewapi"
else
