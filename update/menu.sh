...
export COLOR1="$(cat /etc/cobainvpn/theme/$colornow | grep -w "TEXT" | cut -d: -f2|sed 's/ //g')"
export COLBG1="$(cat /etc/cobainvpn/theme/$colornow | grep -w "BG" | cut -d: -f2|sed 's/ //g')"                    
###########- END COLOR CODE -##########
...
Exp=$(curl -sS https://raw.githubusercontent.com/keluargacobain/permission/main/ipmini | grep $MYIP | awk '{print $3}')
fi
export RED='\033[0;31m'
...
echo "IP=$host" > /var/lib/cobainvpn-pro/ipvps.conf
echo ""
...
sleep 2
wget -q -O /root/install_up.sh "https://raw.githubusercontent.com/keluargacobain/update/main/install_up.sh" && chmod +x /root/install_up.sh
sleep 2
./install_up.sh
sleep 5
rm /root/install_up.sh
rm /opt/.ver
version_up=$( curl -sS https://raw.githubusercontent.com/keluargacobain/update/main/version_up)
...
serverV=$( curl -sS https://raw.githubusercontent.com/keluargacobain/update/main/version_up)
if [ "$Isadmin" = "ON" ]; then
